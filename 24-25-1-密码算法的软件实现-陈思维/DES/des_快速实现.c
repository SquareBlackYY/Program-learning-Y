 ...
 */
void des_key_schedule(uint64_t seed_key, uint64_t round_key[]);
 ...
 * @return 加密得到的密文
 */
//uint64_t des_encrypt(uint64_t m, uint64_t round_key[]);

int main()
{
    const uint64_t seed_key = 0b1000000000100000000100000100100000000000000000100000000001000000ull;
    uint64_t round_key[16] = {0};

    //密钥生成
    des_key_schedule(seed_key, round_key);

    for(int i = 0; i < 16; ++i)
    {
        for (int j = 47; j >= 0; --j)
            printf(round_key[i] >> j & 1 ? "1" : "0");
        printf("\n");
    }
