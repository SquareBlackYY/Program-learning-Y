...
void des_encrypt(uint64_t *pt, const uint64_t rk[], uint64_t *ct)
{
    ...
    for (int r = 0; r < 16; r++)
    {
        uint64_t temp = (ET[0][(*D) >> 24] | ET[1][(*D) >> 16 & 0xFF] | ET[2][(*D) >> 8 & 0xFF] | ET[3][(*D) & 0xFF]) ^ (flag ? rk[15 - r] : rk[r]);
        *next = SPT[0][temp >> 42 & 0x3F] | SPT[1][temp >> 36 & 0x3F] | SPT[2][temp >> 30 & 0x3F] | SPT[3][temp >> 24 & 0x3F] | SPT[4][temp >> 18 & 0x3F] | SPT[5][temp >> 12 & 0x3F] | SPT[6][temp >> 6 & 0x3F] | SPT[7][temp & 0x3F];
        *next ^= *C;
