#include <stdio.h>
#include <stdint.h>
#include <string.h>
#include <time.h>

#define BLOCK_SIZE (1ULL << 20) // 测试分组长度

static const uint32_t FK[4] = {
    0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
};

static const uint32_t CK[32] = {
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};

static const uint8_t S_BOX[256] = {
    
};

// 非线性变换 τ
inline static uint32_t tau(const uint32_t x) {
    return ((uint32_t)S_BOX[x >> 24] << 24) | ((uint32_t)S_BOX[(x >> 16) & 0xFF] << 16) | ((uint32_t)S_BOX[(x >> 8) & 0xFF] << 8) | S_BOX[x & 0xFF];
}

inline static uint32_t L(const uint32_t x) {
    return x ^ (x << 2 | x >> 30) ^ (x << 10 | x >> 22) ^ (x << 18 | x >> 14) ^ (x << 24 | x >> 8);
}

inline static uint32_t L_prime(const uint32_t x) {
    return x ^ (x << 13 | x >> 19) ^ (x << 23 | x >> 9);
}

inline static uint32_t T(const uint32_t x) {
    return L(tau(x));
}

inline static uint32_t T_prime(const uint32_t x) {
    return L_prime(tau(x));
}

static uint32_t Table[4][256];     // T 函数加速表

void InitTable() {
    for (int i = 0; i < 256; i++) {
        uint32_t out = S_BOX[i];
        Table[0][i] = L(out << 24);
        Table[1][i] = L(out << 16);
        Table[2][i] = L(out << 8);
        Table[3][i] = L(out);
    }
}

void GenerateKeySchedule(const uint32_t MK[4], uint32_t rk[32]) {
    uint32_t K[36];
    K[0] = MK[0] ^ FK[0];
    K[1] = MK[1] ^ FK[1];
    K[2] = MK[2] ^ FK[2];
    K[3] = MK[3] ^ FK[3];
    for (int i = 0; i < 32; i++)
        rk[i] = K[i + 4] = K[i] ^ T_prime(K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ CK[i]);
}

void SM4Encrypt(const uint32_t plaintext[4], const uint32_t rk[32], uint32_t ciphertext[4]) {
    static uint32_t X[36];
    memcpy(X, plaintext, 16);
    for (int i = 0; i < 32; i++) {
        uint32_t tmp = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i];
        X[i + 4] = X[i] ^ Table[0][tmp >> 24] ^ Table[1][tmp >> 16 & 0xFF] ^ Table[2][tmp >> 8 & 0xFF] ^ Table[3][tmp & 0xFF];
    }
    // 反序变换
    ciphertext[0] = X[35];
    ciphertext[1] = X[34];
    ciphertext[2] = X[33];
    ciphertext[3] = X[32];
}

void SM4Decrypt(const uint32_t ciphertext[4], const uint32_t rk[32], uint32_t plaintext[4]) {
    uint32_t X[36];
    memcpy(X, ciphertext, 16);
    for (int i = 0; i < 32; i++) {
        uint32_t tmp = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[31 - i];
        X[i + 4] = X[i] ^ Table[0][tmp >> 24] ^ Table[1][tmp >> 16 & 0xFF] ^ Table[2][tmp >> 8 & 0xFF] ^ Table[3][tmp & 0xFF];
    }
    // 反序变换
    plaintext[0] = X[35];
    plaintext[1] = X[34];
    plaintext[2] = X[33];
    plaintext[3] = X[32];
}

int main() {
    InitTable();

    uint32_t MK[4] = {0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210};
    uint32_t rk[32];

    GenerateKeySchedule(MK, rk);
    
    uint32_t plaintext[4] = {0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210};
    uint32_t ciphertext[4], decryptedtext[4];

    printf("明文: ");
    for (int i = 0; i < 4; i++)
        printf("%08x ", plaintext[i]);
    printf("\n");

    // 加密
    clock_t start = clock();
    for (int i = 0; i < BLOCK_SIZE; i++)
        SM4Encrypt(plaintext, rk, ciphertext);
    clock_t end = clock();

    int data = 16 * BLOCK_SIZE / 1024 / 1024;
    printf("加密 %d MB 数据，速度 : %lf Mbps\n", data, 8 * data / ((double)(end - start) / CLOCKS_PER_SEC));

    printf("密文: ");
    for (int i = 0; i < 4; i++)
        printf("%08x ", ciphertext[i]);
    printf("\n");

    // 解密
    SM4Decrypt(ciphertext, rk, decryptedtext);
    
    printf("解密: ");
    for (int i = 0; i < 4; i++)
        printf("%08x ", decryptedtext[i]);
    printf("\n");

    return 0;
}