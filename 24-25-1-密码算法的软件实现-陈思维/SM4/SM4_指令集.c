#include <stdio.h>
#include <stdint.h>
#include <string.h>
#include <time.h>
#include <arm_neon.h>

#define BLOCK_SIZE (1ULL << 20) // 测试分组长度

static const uint32_t FK[4] = {
    0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
};

static const uint32_t CK[32] = {
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};

static const uint8_t S_BOX[256] = {
    
};

// 非线性变换 τ
inline static uint32_t tau(const uint32_t x) {
    return ((uint32_t)S_BOX[x >> 24] << 24) | ((uint32_t)S_BOX[(x >> 16) & 0xFF] << 16) | ((uint32_t)S_BOX[(x >> 8) & 0xFF] << 8) | S_BOX[x & 0xFF];
}

inline static uint32_t L(const uint32_t x) {
    return x ^ (x << 2 | x >> 30) ^ (x << 10 | x >> 22) ^ (x << 18 | x >> 14) ^ (x << 24 | x >> 8);
}

inline static uint32_t L_prime(const uint32_t x) {
    return x ^ (x << 13 | x >> 19) ^ (x << 23 | x >> 9);
}

inline static uint32_t T(const uint32_t x) {
    return L(tau(x));
}

inline static uint32_t T_prime(const uint32_t x) {
    return L_prime(tau(x));
}

inline static void Lookup(const uint32x4_t *x, const uint32_t *t, uint32x4_t *y) {
    static uint32_t idx[4], res[4];

    vst1q_u32(idx, *x);

    res[0] = t[idx[0]];
    res[1] = t[idx[1]];
    res[2] = t[idx[2]];
    res[3] = t[idx[3]];

    *y = vld1q_u32(res);
}

static uint32_t Table[4][256];     // T 函数加速表

void InitTable() {
    for (int i = 0; i < 256; i++) {
        uint32_t out = S_BOX[i];
        Table[0][i] = L(out << 24);
        Table[1][i] = L(out << 16);
        Table[2][i] = L(out << 8);
        Table[3][i] = L(out);
    }
}

void GenerateKeySchedule(const uint32_t MK[4], uint32_t rk[32]) {
    uint32_t K[36];
    K[0] = MK[0] ^ FK[0];
    K[1] = MK[1] ^ FK[1];
    K[2] = MK[2] ^ FK[2];
    K[3] = MK[3] ^ FK[3];
    for (int i = 0; i < 32; i++)
        rk[i] = K[i + 4] = K[i] ^ T_prime(K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ CK[i]);
}

void SM4Encrypt(const uint32_t plaintext[16], const uint32_t rk[32], uint32_t ciphertext[16]) {
    static uint32x4_t X[5], Y[4], round_key, tmp, mask;
    mask = vdupq_n_u32(0xff);

    // 将 4 组明文存入 128-bit 寄存器
    for (int i = 0; i < 4; i++) {
        const uint32_t tmp[4] = {plaintext[i], plaintext[i + 4], plaintext[i + 8], plaintext[i + 12]};
        X[i] = vld1q_u32(tmp);
    }

    for (int i = 0; i < 32; i++) {
        // 每轮密钥扩展到 128 bit
        round_key = vdupq_n_u32(rk[i]);

        // X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i]
        X[(i + 4) % 5] = veorq_u32(veorq_u32(veorq_u32(X[(i + 1) % 5], X[(i + 2) % 5]), X[(i + 3) % 5]), round_key);

        // 查表
        tmp = vshrq_n_u32(X[(i + 4) % 5], 24);
        Lookup(&tmp, Table[0], &Y[0]);

        tmp = vandq_u32(vshrq_n_u32(X[(i + 4) % 5], 16), mask);
        Lookup(&tmp, Table[1], &Y[1]);

        tmp = vandq_u32(vshrq_n_u32(X[(i + 4) % 5], 8), mask);
        Lookup(&tmp, Table[2], &Y[2]);

        tmp = vandq_u32(X[(i + 4) % 5], mask);
        Lookup(&tmp, Table[3], &Y[3]);

        X[(i + 4) % 5] = veorq_u32(veorq_u32(veorq_u32(veorq_u32(Y[0], Y[1]), Y[2]), Y[3]), X[i % 5]);
    }

    // 获取加密结果
    for (int i = 0; i < 4; i++) {
        uint32_t tmp[4];
        vst1q_u32(tmp, X[(35 - i) % 5]);
        ciphertext[i] = tmp[0];
        ciphertext[i + 4] = tmp[1];
        ciphertext[i + 8] = tmp[2];
        ciphertext[i + 12] = tmp[3];
    }
}

void SM4Decrypt(const uint32_t ciphertext[16], const uint32_t rk[32], uint32_t plaintext[16]) {
    static uint32x4_t X[5], Y[4], round_key, tmp;
    const uint32x4_t mask = vdupq_n_u32(0xff);

    // 将 4 组明文存入 128-bit 寄存器
    for (int i = 0; i < 4; i++) {
        const uint32_t tmp[4] = {ciphertext[i], ciphertext[i + 4], ciphertext[i + 8], ciphertext[i + 12]};
        X[i] = vld1q_u32(tmp);
    }

    for (int i = 0; i < 32; i++) {
        // 每轮密钥扩展到 128 bit
        round_key = vdupq_n_u32(rk[31 - i]);

        // X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i]
        X[(i + 4) % 5] = veorq_u32(veorq_u32(veorq_u32(X[(i + 1) % 5], X[(i + 2) % 5]), X[(i + 3) % 5]), round_key);

        // 查表
        tmp = vshrq_n_u32(X[(i + 4) % 5], 24);
        Lookup(&tmp, Table[0], &Y[0]);

        tmp = vandq_u32(vshrq_n_u32(X[(i + 4) % 5], 16), mask);
        Lookup(&tmp, Table[1], &Y[1]);

        tmp = vandq_u32(vshrq_n_u32(X[(i + 4) % 5], 8), mask);
        Lookup(&tmp, Table[2], &Y[2]);

        tmp = vandq_u32(X[(i + 4) % 5], mask);
        Lookup(&tmp, Table[3], &Y[3]);

        X[(i + 4) % 5] = veorq_u32(veorq_u32(veorq_u32(veorq_u32(Y[0], Y[1]), Y[2]), Y[3]), X[i % 5]);
    }

    // 获取加密结果
    for (int i = 0; i < 4; i++) {
        uint32_t tmp[4];
        vst1q_u32(tmp, X[(35 - i) % 5]);
        plaintext[i] = tmp[0];
        plaintext[i + 4] = tmp[1];
        plaintext[i + 8] = tmp[2];
        plaintext[i + 12] = tmp[3];
    }
}

int main() {
    InitTable();

    uint32_t MK[4] = {0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210};
    uint32_t rk[32];

    GenerateKeySchedule(MK, rk);
    
    uint32_t plaintext[16] = {
        0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210, 
        0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210,
        0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210,
        0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210};
    uint32_t ciphertext[16], decryptedtext[16];

    printf("明文:\n");
    for (int i = 0; i < 16; i++) {
        printf("%08x ", plaintext[i]);
        if ((i + 1) % 4 == 0)
            printf("\n");
    }
    
    // 加密
    clock_t start = clock();
    for (int i = 0; i < BLOCK_SIZE / 4; i++)
        SM4Encrypt(plaintext, rk, ciphertext);
    clock_t end = clock();

    int data = 16 * BLOCK_SIZE / 1024 / 1024;
    printf("加密 %d MB 数据，速度 : %lf Mbps\n", data, 8 * data / ((double)(end - start) / CLOCKS_PER_SEC));

    printf("密文:\n");
    for (int i = 0; i < 16; i++) {
        printf("%08x ", ciphertext[i]);
        if ((i + 1) % 4 == 0)
            printf("\n");
    }

    // 解密
    SM4Decrypt(ciphertext, rk, decryptedtext);
    
    printf("解密:\n");
    for (int i = 0; i < 16; i++) {
        printf("%08x ", decryptedtext[i]);
        if ((i + 1) % 4 == 0)
            printf("\n");
    }

    return 0;
}