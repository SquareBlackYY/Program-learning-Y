...
extern void init_round_keys(uint8x16_t *rk, const uint8_t *k);
extern void aes128_decrypt_neon(const uint8_t *c, uint8_t *p, const uint8x16_t *rk);
...
int main() {
    FILE *input = fopen("ciphertext.bin", "rb");
    FILE *output = fopen("plaintext_decrypted.bin", "wb");
    ...
    printf("解密完成并输出到plaintext_decrypted.bin\n");
    return 0;
}
