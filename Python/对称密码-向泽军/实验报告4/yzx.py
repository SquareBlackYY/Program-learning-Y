...
def textgroup(p):
    return [[p[4 * i + j] for i in range(4)] for j in range(4)]


def grouparray(p):
    return [p[j][i] for i in range(4) for j in range(4)]
        ...
        for j in range(4):
            ...
            for k in range(4):
                if matrix[i][k] == 1:
                    ...
                    sum ^= ((p[k][j] << 1) - (p[k][j] >> 7) * 256) ^ ((p[k][j] >> 7) * 0x1B)
                elif matrix[i][k] == 3:
                    sum ^= (((p[k][j] << 1) - (p[k][j] >> 7) * 256) ^ ((p[k][j] >> 7) * 0x1B)) ^ p[k][j]
            row.append(sum)
        mixarray.append(row)
    return mixarray
...
def keygroup(key):
    return [[key[4 * i + j] for j in range(4)] for i in range(4)]
...
def key_schedule(key):
    ...
    k = int16array(key)
    k = keygroup(k)
    subkeys.append(key_transpose(k))
    for i in range(10):
        F = g_convert(k[3], i)
        k = key_extend(k, F)
        subkeys.append(key_transpose(k))
...
def inv_shiftRows(c):
    return [[c[i][(j - i) % 4] for j in range(4)] for i in range(4)]
...
def inv_subBytes(c):
    ...
    for i in range(4):
        for j in range(4):
            c[i][j] = inv_sbox[c[i][j] // 16][c[i][j] % 16]
    return c


def inv_mixColumns(c):
    inv_matrix = [
        [0x0e, 0x0b, 0x0d, 0x09],
        [0x09, 0x0e, 0x0b, 0x0d],
        [0x0d, 0x09, 0x0e, 0x0b],
    ...
    for i in range(4):
        ...
        for j in range(4):
            ...
            for k in range(4):
                ...
                if inv_matrix[i][k] == 9:
                    sum ^= tmp ^ c[k][j]
                elif inv_matrix[i][k] == 11:
                    sum ^= tmp ^ (((c[k][j] << 1) - (c[k][j] >> 7) * 256) ^ ((c[k][j] >> 7) * 0x1B)) ^ c[k][j]
                elif inv_matrix[i][k] == 13:
                    middle = c[k][j]
                    for n in range(2):
                        middle = ((middle << 1) - (middle >> 7) * 256) ^ ((middle >> 7) * 0x1B)  # 04 * A
                    sum ^= tmp ^ middle ^ c[k][j]
                elif inv_matrix[i][k] == 14:
                    middle = c[k][j]
                    for n in range(2):
                        middle = ((middle << 1) - (middle >> 7) * 256) ^ ((middle >> 7) * 0x1B)  # 04 * A
                    sum ^= tmp ^ middle ^ (((c[k][j] << 1) - (c[k][j] >> 7) * 256) ^ ((c[k][j] >> 7) * 0x1B))
            row.append(sum)
        mixarray.append(row)
    return mixarray


def decrypt_round_function(c, key):
    c = inv_shiftRows(c)
    c = inv_subBytes(c)
    c = key_xor(c, key)
    c = inv_mixColumns(c)
