        ...
        for j in range(4):
            ...
            for k in range(4):
                if MIXCOLUMNS_MATRIX[i][k] == 1:
                    ...
                    num ^= ((text[k][j] << 1) - (text[k][j] >> 7) * 256) ^ ((text[k][j] >> 7) * 0x1B)
                elif MIXCOLUMNS_MATRIX[i][k] == 3:
                    num ^= (((text[k][j] << 1) - (text[k][j] >> 7) * 256) ^ ((text[k][j] >> 7) * 0x1B)) ^ text[k][j]
            row.append(num)
        result.append(row)
    return result
...
decrypted_text = AES_decrypt(int(cipher_text, 16), seed_key)
