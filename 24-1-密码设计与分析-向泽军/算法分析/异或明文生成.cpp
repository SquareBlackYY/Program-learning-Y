// Generating_random_numbers_AES.cpp
...
int main()
{
    ...
    for (int j = 0; j < N8; j++)
    {
        for (int i = 0; i < N23; i += 2) // Increment by 2 to fill only even indices
        {
            if ((i & 0x1) == 0)
            {
            ...
            *(unsigned long long *)plaintext[i] = *(randnp + (i & 0x1));
            if (i + 1 < N23)
            {
                unsigned long long xor_value = *(unsigned long long *)plaintext[i] ^ *(unsigned long long *)xorMask;
                *(unsigned long long *)plaintext[i + 1] = xor_value;
            }
        }
