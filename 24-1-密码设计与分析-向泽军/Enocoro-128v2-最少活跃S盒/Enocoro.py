...
ROUND = 97
SBOX = [2, 7, 16, 29]
XOR = [[31, 32, 34], [32, 2, 35], [33, 7, 36], [35, 36], [37, 16, 32], [38, 29, 33], [2, 6, 3], [7, 15, 8], [16, 28, 17]]
...
for round in range(ROUND):
    ...
    with open("enocoro.lp", "w") as f:
        ...
        for _ in range(round):
            ...
            # 一轮结束后刷新寄存器状态，截取前33位元素
            LFSR_B = LFSR_B_next.copy()[:34]
            get_obj()
        ...
        f.write("Subject To\n")
        # 拒绝初始状态全零
        f.write(" + ".join(["_".join(["b", str(i)]) for i in range(34)]) + " >= 1\n")
        f.writelines(i + "\n" for i in CONSTRAINS)
    ...
    model = gp.read("enocoro.lp", env = gp.Env(params={"OutputFlag":0}))
    model.optimize()
    best_obj = int(model.objVal)
    print(f"{round + 1}\t{best_obj}")