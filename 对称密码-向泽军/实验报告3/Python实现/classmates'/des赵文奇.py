...
class AllDES():
    ...
    #置换
    def __displace(self,table: str,self_table: list) -> str:
        self_result = ''
        for i in self_table:
            self_result += table[i - 1]
        return self_result

    #明文转换二进制
    def strtobin(self,string: str) -> str:
        plat_list = list(bytes(string,'utf-8'))
        result = []
        for i in plat_list:
            result.append(bin(i)[2:].zfill(8))
        return "".join(result)

    #二进制转明文
    def bintostr(self,binary: str) -> str:
        list_bin = [binary[i:i+8] for i in range(0, len(binary), 8)]
        list_int = []
        for num in list_bin:
            list_int.append(int(num,2))
        return list_int
    ...
    #明文二进制切分
    def _get_cut_list(self,binary: str) -> list:
    ...
    def input_secrekey(self):
        key = input("请输入新的密钥")
        if len(key) != 16:
            print("密钥长度不符合，请重新输入：")
            self.input_secrekey()
        else:
            bin_key = self.hex_to_bin(key)
            self.K = bin_key
            print("当前二进制形式密钥为:{}".format(self.K))

    #F结构
    def f_fucition(self,right: str,key: str):
        #对右半部分进行E表扩展
        result = self.__displace(right, self.E)
        #与密钥k异或
    ...
    def __get_key_list(self):
        key = self. __displace(self.K,self.k1)
        left_key = key[0:28]
        right_key = key[28:56]
        keys = []
        for i in range(1, 17):
            ...
            ki = self. __displace(move_key, self.k2)
            keys.append(ki)
        return keys

    #异或计算器
    def __xor_funcition(self,xor1: str, xor2: str):
        size = len(xor1)
        result = ""
    ...
    def __iteration(self, bin_plaintext: str, key_list: list):
        ...
        for i in range(0, 16):
            next_lift = right
            f_result = self.f_fucition(right, key_list[i])
            next_right = self.__xor_funcition(left, f_result)
            left = next_lift
            right = next_right
    ...
    def encode(self, plaintext):
        bin_plaintext = self.hex_to_bin(plaintext)
        bin_plaintext_block = self._get_cut_list(bin_plaintext)
        ciphertext_bin_list = []
        key_list = self.__get_key_list()
        for block in bin_plaintext_block:
            # 初代ip置换
            sub_ip = self.__displace(block, self.ip)
            ite_result = self.__iteration(sub_ip, key_list)
            # 逆ip置换
            sub_ip1 = self.__displace(ite_result, self.ip1)
