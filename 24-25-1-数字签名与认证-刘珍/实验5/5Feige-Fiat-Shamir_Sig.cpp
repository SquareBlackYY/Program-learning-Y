...
#define K 6                         // 比特向量长度
#define INTK_MAX ((1 << K) - 1)     // 只保留 K 位数
...
private:
    mpz_class p, q, n;
    std::vector<mpz_class> s, v;

public:
    User(const size_t& bitLength = 256, const mpz_class& pVal = 0, const mpz_class& qVal = 0, const std::vector<mpz_class> &sArray = {});
    ...
    std::vector<mpz_class> getS() const;
};
...
// 用户类构造函数
User::User(const size_t& bitLength, const mpz_class& pVal, const mpz_class& qVal, const std::vector<mpz_class> &sArray)
    : p(pVal), q(qVal), s(K), v(K) {
    if (pVal == 0) p = generatePrime(bitLength);
    if (qVal == 0) q = generatePrime(bitLength);
    n = p * q;
    if (sArray.empty()) {
        // 如果 sArray 为空，则生成长度为 K 的随机值并计算 v
        for (int i = 0; i < K; ++i) {
            s[i] = getRandomNumber(1, n - 1);
            v[i] = powMod(s[i], 2, n);
        }
    } else {
        s = sArray;  // 如果传入了 sArray，则使用它
        for (int i = 0; i < K; ++i) {
            v[i] = powMod(s[i], 2, n);
        }
    }
...
bool User::verifySignature(User& prover, const char* message, const Signature& sig) {
    ...
    for (int i = 0; i < K; ++i) {
        num = c >> (K - i - 1) & 1;
        if (num.get_ui())
            res = res * prover.v[i] % n;
    }

    return powMod(sig.y, 2, n) == res;
}
...
// 获取 s
std::vector<mpz_class> User::getS() const { return s; }
